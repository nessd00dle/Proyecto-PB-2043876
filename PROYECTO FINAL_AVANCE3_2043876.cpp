...
void Alta()
{
	...
	for (int i = 0; i < alta; i++)
	{
		...
		printf("%cIngrese las caracter%csticas del art%cculo \n", 16, 161, 161);
...
void modificar()
{
	...
	case 10:
		...
		{
			while (getchar() != '\n');
			printf("%cIngrese el n%cmero de unidades del art%cculo\n", 9, 163, 161);
			scanf_s("%d", &TIENDA[i].numunidades);

		}
		break;
	default:
		printf("%cIngrese una opci%cn v%clida\n", 9, 162, 160);
		return modificar();
		break;
	}
...
void listas()
{
	int op3;
	string palbracve;
	printf("\t%cC%cmo le gustar%ca filtrar la informaci%cn%c\n1.Categor%ca/ Clase\n2.G%cnero\n3.Consola\n4.Sin filtro", 168, 162, 161, 162, 63, 161, 130);
	scanf_s("%d", &op3);
	cin.ignore();
	printf("%c Ingrese la palabra por el cual quiere buscar:\n",176);
	getline(cin, palbracve);
...
void validItem(int nuevoItem, bool& Repetido, int nArticulos) //Validamos el ITEM necesita 3 parámetros enteros,booleano y el número deL ARTÍCULO
{
